...
function viewAllEmployees() {
    connection.query(
        `SELECT employee.employeeName AS NAME, employee.id, employee.managerID AS MANAGER, 
        role.title AS "JOB TITLE", role.salary AS SALARY, department.departmentName AS DEPT 
        FROM((employee 
          ...
          connection.query(addRoleQuery, (err, results) => {
            ...
            .then(function (answers) {
              ...
              connection.query(roleQuery2, [0, answers.title, answers.salary, answers.departmentID], function (err) {
                if (err) throw err;
                console.log(`${answers.title} added as new role in the ${answers.departmentName} department.`)
                startPrompt();
              })    
            })
          })
